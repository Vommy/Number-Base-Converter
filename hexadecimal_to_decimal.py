def hexadecimal_to_decimal(user_input_number):
    import menu
    hex_0 = [0, 0, 0, 0]
    hex_1 = [0, 0, 0, 1]
    hex_2 = [0, 0, 1, 0]
    ...
    if not actual_hex_number:
        ...
        hexadecimal_to_decimal(user_input_number)
    else:
        ...
        for i in range(0, len(binary_ans), 1):
            binary_ans[i] = binary_ans[i] * (2**i)
        decimal_ans = sum(binary_ans)
        print("Answer = {0} \n".format(decimal_ans))

        def exitLoop5():
            import time
            # Ask the user if they would like to convert another decimal number to hexadecimal
            answer = input("Would you like to convert another hexadecimal number to decimal? (Y/N) \n")
